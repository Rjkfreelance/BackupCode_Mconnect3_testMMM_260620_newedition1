    ...
    if (!writeFile(SD, filenamesavesd , datasavesdcard)) {
      ...
      //For test RJK
       ...
       if(write_addeeprom == 0){
         int n =0;
          while(n < xncount){
          Serial.print(xdataContinue[n]);
          for (int m = 0; m < addrsize; m++)
            {
              
              mac.writeAddress(write_addeeprom,xdataContinue[n].c_str()[m]);
              write_addeeprom++;
...
/*----------------Declare Tasks---------------------*/
TridentOS   task_CheckWiFiMqtt,task_packdata;
            
   void taskCheckWiFi_and_Mqtt(void*),taskPackdatcon(void*);
...
void setup() {
  ...
  client.setCallback(IOTCallback);
   ...
   //task_packdata.start(taskPackdatcon,NULL,115200);//Not work packdata between Write SDcard
   //task_packdata.start(taskPackdatcon,NULL,112400);//Work packdata but waitfor packdata
   //  task_packdata.start(taskPackdatcon,NULL,113000);//Work packdata but waitfor packdata
